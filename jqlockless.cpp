...
struct JqThreadLocalState
{
	JqSelfStack JqSelfStack[JQ_MAX_JOB_STACK];
	uint32_t	JqSelfPos;
	uint32_t	JqNumPipes[JQ_NUM_PIPES];
	JqJobStack* JqJobStacks;
};

JQ_THREAD_LOCAL JqThreadLocalState JqLocalState;

#define JQ_PIPE_EXTRA_BITS 0x4
#define JQ_PIPE_EXTRA_MASK ((1 << JQ_PIPE_EXTRA_BITS) - 1)
...
static_assert(JQ_PIPE_EXTRA_MASK == 0xf, "fisk");
uint64_t JqUnpackExtra(uint64_t Handle)
{
	return Handle & JQ_PIPE_EXTRA_MASK;
}
uint64_t JqUnpackHandle(uint64_t Handle)
{
	return Handle & (~JQ_PIPE_EXTRA_MASK);
}
uint64_t JqHandleIndex(uint64_t Handle)
{
	static_assert((JQ_PIPE_BUFFER_SIZE & (JQ_PIPE_BUFFER_SIZE - 1)) == 0, "JQ_PIPE_BUFFER_SIZE must be power of 2");
	return (Handle >> JQ_PIPE_EXTRA_BITS) & (JQ_PIPE_BUFFER_SIZE - 1)
}

uint64_t JqPackHandle(uint64_t Handle, uint64_t Extra)
{
	JQ_ASSERT((Handle ^ JQ_PIPE_EXTRA_MASK) == 0);
	JQ_ASSERT((Extra ^ (~JQ_PIPE_EXTRA_MASK)) == 0);
	return Handle | Extra;
}

struct JqJobStateInternal
{
	uint64_t SubState : 3;
	uint64_t HasWaiters : 1;
	uint64_t Pad : 12;
	uint64_t NumFinished : 16;
	uint64_t NumChildren : 16;
	uint64_t NumFinishedChildren : 16;
	uint64_t FinishedHandle : 64;
};

struct JQ_ALIGN_CACHELINE JqJob3
{
	JqFunction						Function;
	uint64_t						Range;
	uint64_t						NumJobs;
	std::atomic<JqJobStateInternal> State;
};
...
struct JQ_ALIGN_CACHELINE JqJobPipe
{
	JqJob3				m_Jobs3[JQ_PIPE_BUFFER_SIZE];
	std::atomic<uint64> m_FinishedHandle;
	std::atomic<uint64> m_StartedHandle;
};
...
struct JQ_ALIGN_CACHELINE JqState_t
{
	...
	JqJobPipe m_Pipes3[JQ_NUM_PIPES];
...
void JqContextRun(JqTransfer T)
{
	...
	JqState.m_Pipes3[pJobData->nPipeIndex3].m_Jobs3[pJobData->nJobIndex3].Function(pJobData->nBegin, pJobData->nEnd);
...
void JqRunInternal(uint32_t nPipeIndex3, uint32_t nJobIndex3, int nBegin, int nEnd, uint32_t nJobFlags)
{
	if(JQ_INIT_USE_SEPERATE_STACK == (JqState.m_Attributes.Flags & JQ_INIT_USE_SEPERATE_STACK))
	{

		JqThreadLocalState& LocalState = JqLocalState;
		bool				bSmall	   = 0 != (nJobFlags & JQ_JOBFLAG_SMALL_STACK);
		uint32_t			nStackSize = bSmall ? JqState.m_Attributes.nStackSizeSmall : JqState.m_Attributes.nStackSizeLarge;
		JqJobStack*			pJobData   = JqAllocStack(JqGetJobStackList(nJobFlags), nStackSize, nJobFlags);
		void*				pHest	   = LocalState.JqJobStacks;
		JQ_ASSERT(pJobData->pLink == nullptr);
		pJobData->pLink		   = LocalState.JqJobStacks;
		pJobData->nBegin	   = nBegin;
		pJobData->nEnd		   = nEnd;
		pJobData->nExternalId  = -1;
		pJobData->nPipeIndex3  = nPipeIndex3;
		pJobData->nJobIndex3   = nJobIndex3;
		LocalState.JqJobStacks = pJobData;
		...
		LocalState.JqJobStacks = pJobData->pLink;
		pJobData->pLink		   = nullptr;
		JQ_ASSERT(pHest == LocalState.JqJobStacks);
		JQ_ASSERT(pJobData->GUARD[0] == 0xececececececececll);
		JQ_ASSERT(pJobData->GUARD[1] == 0xececececececececll);
		JqFreeStack(JqGetJobStackList(nJobFlags), pJobData);
	}
	else
	{
		JqState.m_Pipes3[nPipeIndex3].m_Jobs3[nJobIndex3].Function(pJobData->nBegin, pJobData->nEnd);
	}
}
...
void JqStart(JqAttributes* pAttr)
{
		...
		if(-1 == nSelectedSemaphore)
		{
			...
			for(uint32_t j = 0; j < JQ_NUM_PIPES; ++j)
			{
				if(PipeMask & (1llu << j))
				{
					...
					JQ_ASSERT(nSelectedSemaphore < 256);
	...
	JqState.nNumWorkers		 = JqState.m_Attributes.nNumWorkers;
	JqState.nStop			 = 0;
...
uint64_t JqDetachChild(uint64_t nChild)
{
	...
	while(ChildState.nDeleted != 0)
	{
		...
		JqJobTreeState PrevState, DeleteState, xx;
		do
		{
...
void JqWorker(int nThreadId)
{
	...
	while(0 == JqState.nStop)
	{
		...
		for(uint32_t i = 0; i < nNumPipes; ++i)
		{
			if(JqExecuteOne(pPipes, (uint8_t)nNumPipes))
			{
				bExecuted = true;
				break;
			}
...
void JqDump(FILE* F, uint32_t nNode, uint32_t nIndent)
{
	...
	fprintf(F, "%08d %lld/%lld, root %lld pipe %lld, pipehandle %lld  PCS, %5d, %5d, %5d\n", nNode, pJob->Handle.load().nStarted, pJob->Handle.load().nFinished, pJob->nRoot.load(),
			pJob->PipeHandle.load().Pipe, pJob->PipeHandle.load().HandleInt, TS.nParent, TS.nFirstChild, TS.nSibling
...
void JqDump()
{
	...
	for(uint32_t i = 0; i < JQ_NUM_PIPES; ++i)
	{
		...
		fprintf(FX, " XXX PIPE DUMP %d XXX\n", i);
		JqPipeDump(FX, &JqState.m_Pipes[i]);
		fclose(FX);
	}
